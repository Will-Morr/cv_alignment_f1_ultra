...
python align.py demo        # burns a border 5 mm inside the workpiece edges and a centred crosshair
```

`calibrate` and `demo` fire the laser. Focus first (`F1Ultra().autofocus()` or the touchscreen),
...
- Autofocus only works after switching the device mode to `P_AUTOFOCUS`.
